    ...
    "support simple round-trips" should {
      abstract class SimpleRequestResponseRoundtripSetup extends TestSetup with RequestResponseProbes {
        def requestResponseRoundtrip(
          streamId: Int,
          request:          HttpRequest,
          expectedData:             ByteString,
          response:                    ByteString,
          expectedResponse: HttpResponse
        ): Unit = {
          emitRequest(streamId, request)

          // TODO this fails, I suspect we send more headers than the example.
          //  I think we should be less strict in the test. In any case we should log the parsed frame in that case.
          expectFrame() match {
            case HeadersFrame(streamId, true, true, headers, _) => //ok
          }
          // TODO support expecting a body
          // TODO send required headers (:status etc)
          sendHEADERS(streamId, endStream = true, endHeaders = true, ByteString.empty)
//          fromNet.sendNext(response)
...
//          expectGracefulCompletion()
      ...
      "GET request in one HEADERS frame" in new SimpleRequestResponseRoundtripSetup {
        requestResponseRoundtrip(
          streamId = 1,
          request = HttpRequest(uri = "http://www.example.com/", headers = Nil),
          expectedData = HPackSpecExamples.C41FirstRequestWithHuffman,
          response = HPackSpecExamples.C61FirstResponseWithHuffman,
...
//    }
  ...
  abstract class TestSetup extends TestSetupWithoutHandshake with RequestResponseProbes {
    toNet.expectBytes(Http2Protocol.ClientConnectionPreface)
    // TODO right now we *require* the settings to be empty, but we want to *allow* it to be empty...
  ...
  trait AutomaticHpackWireSupport extends RequestResponseProbes {
  ...
  /** Provides the net flow as `requestIn` and `responseOut` probes for manual stream interaction */
  trait RequestResponseProbes extends TestSetupWithoutHandshake with Http2FrameProbeDelegator {
