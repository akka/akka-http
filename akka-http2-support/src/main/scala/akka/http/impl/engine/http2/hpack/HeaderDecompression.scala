  ...
  def createLogic(inheritedAttributes: Attributes): GraphStageLogic = new HandleOrPassOnStage[FrameEvent, FrameEvent](shape) {
    ...
    def parseAndEmit(streamId: Int, endStream: Boolean, payload: ByteString): Unit = {
      ...
      try {
        ...
        case ex: IOException ⇒
          // this is signalled by the decoder when it failed, we want to react to this by rendering a GO_AWAY frame
          push(eventsOut, GoAwayFrame(streamId, ErrorCode.COMPRESSION_ERROR))
