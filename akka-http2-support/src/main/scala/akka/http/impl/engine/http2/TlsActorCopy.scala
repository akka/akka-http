...
class TLSActor(
  ...
  private def doUnwrap(ignoreOutput: Boolean = false): Unit = {
    ...
    lastHandshakeStatus = result.getHandshakeStatus
    if (tracing) log.debug(s"unwrap: status=${result.getStatus} handshake=$lastHandshakeStatus remaining=${transportInBuffer.remaining} out=${userOutBuffer.position} bytesConsumed=${result.bytesConsumed()}")
    runDelegatedTasks()
    result.getStatus match {
      case OK ⇒
        result.getHandshakeStatus match {
          ...
          case _ ⇒
            if (transportInBuffer.hasRemaining || (result.bytesProduced() > 0)) doUnwrap()
            else flushToUser()
        }
      case CLOSED ⇒
        flushToUser()
