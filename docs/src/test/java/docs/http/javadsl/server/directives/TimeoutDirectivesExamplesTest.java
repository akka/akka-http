...
public class TimeoutDirectivesExamplesTest extends AllDirectives {
  ...
  private Optional<HttpResponse> runRoute(
      ActorSystem system, ActorMaterializer materializer, Route route, String routePath) {
    final Tuple2<String, Object> inetaddrHostAndPort =
        SocketUtil.temporaryServerHostnameAndPort("127.0.0.1");
    Tuple2<String, Integer> hostAndPort =
        new Tuple2<>(inetaddrHostAndPort._1(), (Integer) inetaddrHostAndPort._2());

    final Flow<HttpRequest, HttpResponse, NotUsed> routeFlow = route.flow(system, materializer);
    final CompletionStage<ServerBinding> binding =
        http.bindAndHandle(
            routeFlow, ConnectHttp.toHost(hostAndPort._1(), hostAndPort._2()), materializer);

    final CompletionStage<HttpResponse> responseCompletionStage =
        http.singleRequest(
            HttpRequest.create(
                "http://" + hostAndPort._1() + ":" + hostAndPort._2() + "/" + routePath),
            materializer);
