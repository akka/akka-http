...
public class JobRepository extends AbstractBehavior<JobRepository.Command> {
    ...
    public static final class GetJobByStatus implements Command {
        final Status status;
        final ActorRef<List<Job>> replyTo;

        public GetJobByStatus(Status status, ActorRef<List<Job>> replyTo) {
            this.status = status;
            this.replyTo = replyTo;
        }
    }
