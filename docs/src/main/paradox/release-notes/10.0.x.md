 ...
 * Java DSL routing `complete` now has override that takes @apidoc[ResponseEntity] as a parameter instead of @apidoc[RequestEntity] ([#982](https://github.com/akka/akka-http/issues/982))
...
##### akka-http2-support
 * Multiplexer infrastructure was rewritten to support plugable @apidoc[StreamPrioritizer] (not yet surfaced in user API) ([f06ab40](https://github.com/akka/akka-http/commit/f06ab40))
