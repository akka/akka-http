...
When combining an alternative expressed using this operator with an `/` operator, make sure to surround the alternative with parentheses, like so: `("foo" | "bar") / "bom"`. Otherwise the `/` operator take precendence and would only apply to the right-hand side of the alternative.
