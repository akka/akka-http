...
class ContentNegotiationSpec extends FreeSpec with Matchers {
  "Content Negotiation should work properly for requests with header(s)" - {
    "(without headers)" test { accept ⇒
      accept(Seq(`text/plain` withCharset `UTF-16`)) should select(`text/plain` withCharset `UTF-16`)
      accept(Seq(`text/plain`, `text/html`)) should select(`text/plain` withCharset `UTF-8`)
      accept(Seq(`text/html`, `text/plain`)) should select(`text/html` withCharset `UTF-8`)
    }

    "Accept: */*" test { accept ⇒
      accept(Seq(`text/plain`)) should select(`text/plain` withCharset `UTF-8`)
      accept(Seq(`text/plain` withCharset `UTF-16`)) should select(`text/plain` withCharset `UTF-16`)
    }
    ...
    "Accept: text/*" test { accept ⇒
      accept(Seq(`text/plain`)) should select(`text/plain` withCharset `UTF-8`)
      accept(Seq(`text/xml` withCharset `UTF-16`)) should select(`text/xml` withCharset `UTF-16`)
      accept(Seq(`audio/ogg`)) should reject
    }
    ...
    "Accept: text/*;q=0" test { accept ⇒
      accept(Seq(`text/plain`)) should reject
      accept(Seq(`text/xml` withCharset `UTF-16`)) should reject
      accept(Seq(`audio/ogg`)) should reject
    }

    "Accept: text/*, application/json;q=0.8, text/plain;q=0.5" test { accept ⇒
      accept(Seq(`text/plain`, `application/json`)) should select(`application/json`)
    }

    "Accept-Charset: UTF-16" test { accept ⇒
      accept(Seq(`text/plain`)) should select(`text/plain` withCharset `UTF-16`)
      accept(Seq(`text/plain` withCharset `UTF-8`)) should reject
    }
    ...
    "Accept-Charset: UTF-16, UTF-8" test { accept ⇒
      accept(Seq(`text/plain`)) should select(`text/plain` withCharset `UTF-16`)
      accept(Seq(`text/plain` withCharset `UTF-8`)) should select(`text/plain` withCharset `UTF-8`)
    }
    ...
    "Accept-Charset: ISO-8859-1;q=.2" test { accept ⇒
      accept(Seq(`text/plain`)) should select(`text/plain` withCharset `ISO-8859-1`)
      accept(Seq(`text/plain` withCharset `UTF-8`)) should reject
    }
    ...
    "Accept-Charset: *;q=0" test { accept ⇒
      accept(Seq(`text/plain`)) should reject
      accept(Seq(`text/plain` withCharset `UTF-16`)) should reject
    }

    "Accept-Charset: *;q=0.1" test { accept ⇒
      accept(Seq(`text/plain`)) should select(`text/plain` withCharset `UTF-8`)
      accept(Seq(`image/gif`)) should select(`image/gif`)
    }

    "Accept-Charset: us;q=0.1,*;q=0" test { accept ⇒
      accept(Seq(`text/plain`)) should select(`text/plain` withCharset `US-ASCII`)
      accept(Seq(`text/plain` withCharset `UTF-8`)) should reject
    }
    ...
    "Accept: text/xml, text/html;q=.5" test { accept ⇒
      accept(Seq(`text/plain`)) should reject
      accept(Seq(`text/xml`)) should select(`text/xml` withCharset `UTF-8`)
      accept(Seq(`text/html`)) should select(`text/html` withCharset `UTF-8`)
      accept(Seq(`text/html`, `text/xml`)) should select(`text/xml` withCharset `UTF-8`)
      accept(Seq(`text/xml`, `text/html`)) should select(`text/xml` withCharset `UTF-8`)
      accept(Seq(`text/plain`, `text/xml`)) should select(`text/xml` withCharset `UTF-8`)
      accept(Seq(`text/plain`, `text/html`)) should select(`text/html` withCharset `UTF-8`)
    }

    """Accept: text/html, text/plain;q=0.8, application/*;q=.5, *;q= .2
      |Accept-Charset: UTF-16""" test { accept ⇒
      accept(Seq(`text/plain`, `text/html`, `audio/ogg`)) should select(`text/html` withCharset `UTF-16`)
      accept(Seq(`text/plain`, `text/html` withCharset `UTF-8`, `audio/ogg`)) should select(`text/plain` withCharset `UTF-16`)
      accept(Seq(`audio/ogg`, `application/javascript`, `text/plain` withCharset `UTF-8`)) should select(`application/javascript` withCharset `UTF-16`)
      accept(Seq(`image/gif`, `application/javascript`)) should select(`application/javascript` withCharset `UTF-16`)
      accept(Seq(`image/gif`, `audio/ogg`)) should select(`image/gif`)
    }

    "Accept: text/xml, text/plain" test { accept ⇒
      accept(Seq(`text/plain` withCharset `UTF-16`)) should select(`text/plain` withCharset `UTF-16`)
      accept(Seq(`text/plain`, `text/xml`)) should select(`text/plain` withCharset `UTF-8`)
      accept(Seq(`text/xml`, `text/plain`)) should select(`text/xml` withCharset `UTF-8`)
    }
    ...
    "Accept: text/html;level=1, text/html;format=flowed" test { accept ⇒
      accept(Seq(`text/html` withParams Map("level" → "1"), `text/html` withParams Map("format" → "flowed"))) should select(`text/html` withParams Map("level" → "1") withCharset `UTF-8`)
    }
    ...
    "Accept: text/html;level=1;q=0.4, text/html;format=flowed;q=0.7" test { accept ⇒
      accept(Seq(`text/html` withParams Map("level" → "1"), `text/html` withParams Map("format" → "flowed"))) should select(`text/html` withParams Map("format" → "flowed") withCharset `UTF-8`)
    }
    ...
    "Accept: text/html;level=1;q=0.4, text/html;format=flowed;q=1, text/html;q=0.7" test { accept ⇒
      accept(Seq(`text/html` withParams Map("level" → "1", "format" → "flowed"), `text/html`)) should select(`text/html` withParams Map("level" → "1", "format" → "flowed") withCharset `UTF-8`)
    }
  ...
  def testHeaders[U](headers: HttpHeader*)(body: ((Seq[Alternative]) ⇒ Option[ContentType]) ⇒ U): U = {
  ...
  implicit class AddStringToIn(example: String) {
    def test(body: ((Seq[Alternative]) ⇒ Option[ContentType]) ⇒ Unit): Unit = example in {
      val headers =
