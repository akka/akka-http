...
import akka.http.impl.engine.server.{ SwitchableIdleTimeoutBidi, SwitchableIdleTimeoutPrototype }
  ...
  def apply(idleTimeout: FiniteDuration, remoteAddress: Option[InetSocketAddress]): BidiFlow[ByteString, ByteString, ByteString, ByteString, NotUsed] = {
    ...
    val ex = new HttpIdleTimeoutException(
      "HTTP idle-timeout encountered" + connectionToString + ", " +
        ...
        "This is configurable by akka.http.server.idle-timeout.", idleTimeout)

    val mapError = Flow[ByteString].mapError {
      case t: TimeoutException ⇒ ex
    }
    ...
    val fromNetTimeout: BidiFlow[ByteString, Either[SwitchableIdleTimeoutBidi.TimeoutSwitch, ByteString], ByteString, ByteString, NotUsed] =
      BidiFlow.fromFlows(
        SwitchableIdleTimeoutPrototype.identifyIdleTimeoutSwitchBytestrings,
        mapError
      )

    fromNetTimeout atop SwitchableIdleTimeoutBidi.bidirectionalSettableIdleTimeout[ByteString, ByteString](idleTimeout) atop toNetTimeout
  }
}
