...
private[http2] class Http2ServerDemux(http2Settings: Http2CommonSettings, initialDemuxerSettings: immutable.Seq[Setting], upgraded: Boolean, isServer: Boolean) extends GraphStage[BidiShape[Http2SubStream, FrameEvent, FrameEvent, Http2SubStream]] { stage =>
  ...
  def createLogic(inheritedAttributes: Attributes): GraphStageLogic =
    new GraphStageLogic(shape) with Http2MultiplexerSupport with Http2StreamHandling with GenericOutletSupport with StageLogging with LogHelper {
      ...
      private[this] var allowReadingOutgoingFrames = true
      override def allowReadingOutgoingFrames(allow: Boolean): Unit = {
        if (allow != allowReadingOutgoingFrames)
          if (allow) {
            debug("Resume reading outgoing frames")
            if (!hasBeenPulled(substreamIn)) pull(substreamIn)
          } else debug("Suspended reading outgoing frames") // can't retract pending pull but that's ok

        allowReadingOutgoingFrames = allow
      }
      def pullSubstreamIn(): Unit = if (allowReadingOutgoingFrames && !hasBeenPulled(substreamIn)) pull(substreamIn)
        ...
        def onPush(): Unit = {
          ...
          in match {
            ...
            case s: StreamFrameEvent =>
              handleStreamEvent(s)
              allowReadingOutgoingFrames(peerHasCapacity)
      ...
      setHandler(substreamIn, new InHandler {
        def onPush(): Unit = {
          val sub: Http2SubStream = grab(substreamIn)
          // pull only if allowed
          pullSubstreamIn()
          handleOutgoingCreated(sub)
          multiplexer.registerSubStream(sub)
          // consider stopping for a bit if we just overflowed the number of streams registered...
          allowReadingOutgoingFrames(peerHasCapacity)
        }
      })
