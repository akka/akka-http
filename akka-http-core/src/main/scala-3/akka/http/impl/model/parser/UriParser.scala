...
private[http] final class UriParser(
  ...
  def parseAuthority(): Authority = rule(authority ~ EOI).run() match {
  ...
  def query: Rule1[Query] = {
    def part(`query-char`: CharPredicate): Rule1[String] =
    ...
    def keyValuePairsWithLimitedStackUsePart: Rule1[Query.Cons] = rule {
      keyValuePair ~> { (key: String, value: String) => Query.Cons(key, value, Query.Empty) }
    }
    def keyValuePairsWithLimitedStackUseBuildPart: Rule[Query.Cons :: HNil, Query.Cons :: HNil] = rule {
      keyValuePair ~> { (prefix: Query.Cons, key: String, value: String) => Query.Cons(key, value, Query.Empty) }
    }
    def reverse(cons: Query.Cons): Query = cons.reverse
    def keyValuePairsWithLimitedStackUse: Rule1[Query] = rule {
      keyValuePairsWithLimitedStackUsePart ~ {
        zeroOrMore('&' ~ keyValuePairsWithLimitedStackUseBuildPart) ~> reverse
      }
    }
